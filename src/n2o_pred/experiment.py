    ...
    def _generate_outputs_rnn(
        ...
        for i in range(len(val_dataset)):
            ...
            with torch.no_grad():
                ...
                static_categorical = seq['static_categorical'].unsqueeze(0).to(self.device)
                dynamic_categorical = seq['dynamic_categorical'].unsqueeze(0).to(self.device)
                seq_lengths = torch.tensor([seq['seq_length']]).to(self.device)
                
                pred_scaled = model(static_numeric, static_categorical,
        ...
        for idx in good_seq_indices:
            ...
            if use_mask:
                ...
                time_steps = np.arange(val_dataset.daily_sequences[idx]['start_day'],
            ...
            with torch.no_grad():
                ...
                dynamic_numeric = seq_data['dynamic_numeric'].unsqueeze(0).to(self.device)
                static_categorical = seq_data['static_categorical'].unsqueeze(0).to(self.device)
                dynamic_categorical = seq_data['dynamic_categorical'].unsqueeze(0).to(self.device)
