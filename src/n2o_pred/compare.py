...
class ModelComparator:
    ...
    def __init__(self, exp_dirs: list[Path | str], output_dir: Path | str | None = None):
        """
        Args:
            exp_dirs: 实验目录列表
            output_dir: 输出目录
        ...
        if output_dir is None:
            ...
            output_dir = Path(__file__).parents[2] / 'outputs' / f'comparison_{timestamp}'
    ...
    def _collect_comparison_data(self) -> dict[str, Any]:
        ...
        for seed in common_seeds:
            ...
            for summary in self.summaries:
                ...
                if seed_result:
                    data['by_seed'][seed]['train_R2'].append(seed_result['metrics']['train']['R2'])
                    data['by_seed'][seed]['val_R2'].append(seed_result['metrics']['val']['R2'])
                    data['by_seed'][seed]['train_RMSE'].append(seed_result['metrics']['train']['RMSE'])
                    data['by_seed'][seed]['val_RMSE'].append(seed_result['metrics']['val']['RMSE'])
    ...
    def _generate_comparison_plots(self, data: dict[str, Any]):
        ...
        for i, metric in enumerate(metrics_to_plot):
            ...
            bars = ax.bar(range(len(data['exp_names'])), values, 
        ...
        if len(data['by_seed']) > 1:
            ...
            for idx, (metric_key, metric_label) in enumerate(metrics_for_box):
                ...
                ax.set_title(f'{metric_label} Distribution Across Seeds', 
                           fontsize=13, fontweight='bold')
                ax.grid(axis='y', alpha=0.3)
            
            plt.tight_layout()
            plt.savefig(figs_dir / 'metrics_distribution.png', dpi=300, bbox_inches='tight')
            plt.close()
        ...
        colors = plt.cm.viridis(np.linspace(0, 1, len(data['exp_names'])))
        for i, exp_name in enumerate(data['exp_names']):
            ax.scatter(data['n_parameters'][i], data['val_metrics']['R2'][i],
                      s=200, c=[colors[i]], alpha=0.7, label=exp_name)
        ...
        plt.tight_layout()
        plt.savefig(figs_dir / 'complexity_vs_performance.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def compare_experiments(exp_dirs: list[Path | str], output_dir: Path | str | None = None) -> dict[str, Any]:
    """
    比较多个实验的便捷函数
