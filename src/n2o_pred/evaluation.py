...
def compute_sequence_metrics(
    predictions_by_seq: list[np.ndarray],
    targets_by_seq: list[np.ndarray]
) -> pd.DataFrame:
    """
    计算每个序列的评估指标
...
def plot_predictions_vs_actual(
    ...
    ax1.set_title(f'Train Set (R²={train_r2:.4f}, RMSE={train_rmse:.4f})', 
    ...
    ax2.set_title(f'Test Set (R²={test_r2:.4f}, RMSE={test_rmse:.4f})', 
                  fontsize=13, fontweight='bold')
...
def plot_sequence_predictions(
    ...
    if mask is not None:
        ...
        plt.plot(time_steps[real_indices], targets[real_indices], 'bo-', 
                label='Actual (Measured)', markersize=6, linewidth=2)
        plt.plot(time_steps[real_indices], predictions[real_indices], 'rs-', 
                label='Predicted (Measured)', markersize=6, linewidth=2, alpha=0.7)
        
        if np.any(interp_indices):
            plt.plot(time_steps[interp_indices], targets[interp_indices], 'b--', 
                    label='Actual (Interpolated)', alpha=0.3, linewidth=1)
            plt.plot(time_steps[interp_indices], predictions[interp_indices], 'r--', 
                    label='Predicted (Interpolated)', alpha=0.3, linewidth=1)
    else:
        plt.plot(time_steps, targets, 'bo-', label='Actual', markersize=6, linewidth=2)
        plt.plot(time_steps, predictions, 'rs-', label='Predicted', markersize=6, linewidth=2, alpha=0.7)
    ...
    plt.title(f'Sequence {seq_id} Predictions (R²={r2:.4f})', fontsize=14, fontweight='bold')
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
...
def compute_shap_values(
    ...
    if model_type == 'rf':
        ...
        for idx in indices[:100]:  # 进一步限制为100个样本以加快速度
            ...
            # 准备输入
            static_numeric = batch['static_numeric'].unsqueeze(0).to(device).requires_grad_(True)
            dynamic_numeric = batch['dynamic_numeric'].unsqueeze(0).to(device).requires_grad_(True)
            ...
            # 前向传播
            outputs = model(static_numeric, static_categorical, dynamic_numeric, 
                          dynamic_categorical, seq_lengths)
            ...
            if dynamic_numeric.grad is not None:
                grad_dynamic = dynamic_numeric.grad.abs().cpu().numpy().mean(axis=1).flatten()
