...
def cmd_train(args):
    ...
    if model_type.startswith("rnn"):
        ...
        runner = ExperimentRunner(
            model_type=model_type, output_dir=output_dir, rf_config=config
        )
...
示例:
  ...
  n2o-pred predict --model outputs/exp_xxx/split_42 --dataset datasets/data_EUR_processed.pkl
    ...
    parser_data.add_argument(
        "--preprocessing", action="store_true", help="执行数据预处理"
    )
    ...
    seed_group.add_argument(
        "--max-split", type=int, default=1, help="随机种子数量（默认1）"
    )
    ...
    parser_train.add_argument(
        "--patience", type=int, default=30, help="早停耐心值（默认30）"
    )
    ...
    parser_compare.add_argument(
        "--models", nargs="+", required=True, help="实验目录列表"
    )
