    ...
    for epoch in range(config.max_epochs):
        ...
        for batch in pbar:
            ...
            if torch.isnan(loss):
                ...
                logger.error(
                    f"预测值范围: [{predictions.min().item():.4f}, {predictions.max().item():.4f}]"
                )
                logger.error(
                    f"目标值范围: [{targets.min().item():.4f}, {targets.max().item():.4f}]"
                )
                continue
        ...
        logger.info(
            f"Epoch {epoch + 1}: train_loss={avg_train_loss:.6f}, val_loss={avg_val_loss:.6f}"
        )
