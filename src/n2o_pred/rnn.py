...
class N2OPredictorRNN(nn.Module):
    ...
    def __init__(
        ...
        # 计算静态特征的总维度
        static_dim = (
            num_numeric_static + len(categorical_static_cardinalities) * embedding_dim
        )
        ...
        # 计算动态特征的总维度
        dynamic_dim = (
            num_numeric_dynamic + len(categorical_dynamic_cardinalities) * embedding_dim
        )
    ...
    def forward(
        ...
        # 4. 输出层
        predictions = self.output_layer(rnn_output).squeeze(
            -1
