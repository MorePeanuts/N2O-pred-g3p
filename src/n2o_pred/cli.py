...
def cmd_predict(args):
    ...
    logger.info("预测完成！")
    if results['metrics']:
        logger.info(f"评估指标: R2={results['metrics']['R2']:.4f}, "
...
示例:
  ...
  n2o-pred predict --model outputs/exp_xxx/split_42 --dataset datasets/data_EUR_processed.pkl
    ...
    parser_train.add_argument('--split-seed', type=int, default=42,
                             help='生成随机种子的种子（默认42）')
    
    # 训练参数
    parser_train.add_argument('--train-split', type=float, default=0.9,
                             help='训练集比例（默认0.9）')
    parser_train.add_argument('--device', type=str, default='cuda:0',
                             help='设备（默认cuda:0）')
    parser_train.add_argument('--output', type=str, help='输出目录')
    
    # RNN参数
    parser_train.add_argument('--max-epochs', type=int, default=300,
                             help='最大训练轮次（默认300）')
    parser_train.add_argument('--batch-size', type=int, default=32,
                             help='批次大小（默认32）')
    parser_train.add_argument('--lr', type=float, default=1e-3,
                             help='学习率（默认0.001）')
    parser_train.add_argument('--patience', type=int, default=30,
                             help='早停耐心值（默认30）')
    ...
    parser_compare.add_argument('--models', nargs='+', required=True,
                               help='实验目录列表')
    ...
    parser_predict.add_argument('--model', type=str, required=True,
                               help='模型目录')
    parser_predict.add_argument('--dataset', type=str, required=True,
                               help='数据路径')
    parser_predict.add_argument('--output', type=str, help='输出路径')
    parser_predict.add_argument('--device', type=str, default='cpu',
                               help='设备（默认cpu）')
